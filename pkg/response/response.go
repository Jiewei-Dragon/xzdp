...
func HandleBusinessError(c *gin.Context, err error) {
	...
	if errors.As(err, &bizErr) {
		//记录业务错误日志
		if bizErr.Err != nil {
			slog.Error("业务错误", "originalErr", bizErr.Err, "code", bizErr.Code, "message", bizErr.Message)
		}
...
func HandleBusinessErrorWithData(c *gin.Context, err error, data any) {
	...
	if errors.As(err, &bizErr) {
		if bizErr.Err != nil {
			slog.Error("业务错误（附带数据）", "originalErr", bizErr.Err, "code", bizErr.Code, "message", bizErr.Message)
		}
		ErrorWithData(c, bizErr.Code, data, bizErr.Message)
	} else {
		slog.Error("未知错误（附带数据）", "err", err)
